...
async def send_telemetry_from_thermostat(device_client, telemetry_msg, val):
...
async def main():
    ...
    # async def putSensorData(ip, port, sensor_type, value, status):

    #     async with aiohttp.ClientSession() as session:

    #         url = """http://{ip}:{port}/peer/putData?sensorType={sensor_type}&value={value}&onOff={status}"""
    #         url = url.format(ip = ip, port = port, sensor_type = sensor_type, value = value, status = status)
    ...
    # global motion_c 
    # async def send_data(motion_count):
    #     conn = http.client.HTTPSConnection("54.204.76.18", 8085)
    #     payload = str(motion_count)
    #     headers = {}
    #     conn.request("PUT", "/peer/putData?sensorType=temperature&value=11&onOff=on", payload, headers)
    ...
    async def send_telemetry():
        ...
        while True:
            motion_count = random.randrange(0,4)
            if motion_count == 0:
                val = False
            else:
                val = True
            ...
            if not max_temp:
                max_temp = motion_count
            elif motion_count > max_temp:
                max_temp = motion_count

            if not min_temp:
                min_temp = motion_count
            elif motion_count < min_temp:
                min_temp = motion_count

            avg_temp_list[current_avg_idx] = motion_count
